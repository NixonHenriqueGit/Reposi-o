...
// Check if Firebase is properly configured with a non-empty projectId
const isFirebaseConfigured = Boolean(
  firebaseConfig &&
  typeof (firebaseConfig as any).projectId === "string" &&
  (firebaseConfig as any).projectId.trim().length > 0
);
...
if (isFirebaseConfigured) {
  try {
    ...
    console.log("[FIREBASE-INIT] Firestore initialized successfully with project:", (firebaseConfig as any).projectId);
  } catch (err) {
    console.warn("[FIREBASE-INIT] Error initializing Firebase:", err);
  }
} else {
  console.log("[FIREBASE-INIT] Firebase project ID is empty/unconfigured. App operating in local storage mode.");
}
...
export function initializeSync() {
  if (!firestoreDb) {
    console.log("[SYNC] Firebase is not configured (projectId is empty). Operating in local storage mode.");
    if (!safeGetItem("sstr_cached_records_v1")) {
      seedLocalStorageDefaults();
    }
    return {
      fastSyncPromise: Promise.resolve(),
      heavySyncPromise: Promise.resolve()
    };
  }
    ...
    try {
      ...
      const managersSnap = await withTimeout(
        getDocs(query(managersCol, limit(1))).catch((err) => {
          console.warn("[FAST-SYNC] Could not fetch managers initial query:", err?.message || err);
          return null;
        }),
        4000
      );
