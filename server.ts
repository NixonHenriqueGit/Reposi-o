...
try {
  if (fs.existsSync(configPath)) {
    ...
    if (rawContent) {
      firebaseConfig = JSON.parse(rawContent);
    }
  }
} catch (e) {
  console.warn("[FIREBASE-ADMIN] Could not parse firebase-applet-config.json:", e);
}
...
try {
  if (firebaseConfig && firebaseConfig.projectId) {
    ...
    console.log("[FIREBASE-ADMIN] Skipping Admin SDK initialization (no projectId provided in config).");
  }
} catch (err: any) {
